...
final class WorkFeedTests: XCTestCase {
    ...
    func testProjectsEverySourceOntoTheSameRowShapeAndRanksNeedsYouFirst() {
        ...
        func row(_ key: String) -> WorkRow? { rows.first { $0.key == key } }
        ...
        XCTAssertEqual(F.count(rows), WorkCounts(needsYou: 2, running: 0, waiting: 1, recurring: 2, agents: 1))
    ...
    func testDecodesLooseRows() throws {
        ...
        """#.data(using: .utf8)!
        struct R: Decodable { var tasks: [WorkFeed.UnifiedRow] }
    ...
    func testLegacySectionNamesLandOnSessionsViews() {
        XCTAssertEqual(AppRouter.section(named: "tasks")?.0, .work)
