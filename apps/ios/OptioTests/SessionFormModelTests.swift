...
/// Mirrors `apps/web/src/components/session-form/model.test.ts` case for case,
/// plus the sentence text for a few drafts and the submit-side helpers.
final class SessionFormModelTests: XCTestCase {
    ...
    func testExitsMachineEventTriggerIsLocalAutomation() {
        XCTAssertEqual(F.deriveKind(local(with(base) { $0.when = .github })), .localBlueprint)
    }
    ...
    func testEventTriggerRunsOnYourMachine() {
        XCTAssertEqual(enabled(F.whereOptions(with(empty) { $0.when = .github })), [.local])
        XCTAssertEqual(F.normalize(with(empty) { $0.when = .linear }).location.runTarget, .local)
    }
    ...
    func testDescribesSchedulesAndMarksMissing() {
        ...
        XCTAssertEqual(text(d), "Running weekdays at 09:00 UTC, a Claude Code session in an Optio pod that exits when done.")
    ...
    func testMoreSentences() {
        ...
        XCTAssertEqual(text(hook), "Started by a webhook at /api/hooks/hook-abc, a Claude Code session in an Optio pod that exits when done.")
        let noPath = with(hook) { $0.trigger.webhookPath = nil }
        XCTAssertEqual(text(noPath), "Started by [a webhook path], a Claude Code session in an Optio pod that exits when done.")
        XCTAssertEqual(F.missingFields(noPath), [.webhook, .prompt])

        let gh = F.normalize(with(empty) { $0.when = .github; $0.withRepo = false })
        XCTAssertEqual(text(gh), "Started by GitHub events, a Claude Code session [a machine] [a directory] that exits when done.")
        XCTAssertEqual(F.missingFields(gh), [.machine, .checkout, .prompt])
        ...
        XCTAssertEqual(text(missingRepo), "Started now, a Claude Code session in an Optio pod [a repo] that opens a PR and exits when done.")
    ...
    func testGenericTriggerAndLocationPayload() {
        XCTAssertNil(F.genericTrigger(empty))
        let sched = F.genericTrigger(with(empty) { $0.trigger = F.TriggerConfig(type: .schedule, cronExpression: " 0 9 * * * ") })
        XCTAssertEqual(sched?.type, "schedule")
        XCTAssertEqual(sched?.config, ["cronExpression": .string("0 9 * * *")])
        let ticket = F.genericTrigger(with(empty) { $0.trigger = F.TriggerConfig(type: .ticket, ticketSource: .linear, ticketLabels: ["bug"]) })
        XCTAssertEqual(ticket?.config, ["source": .string("linear"), "labels": .array([.string("bug")])])
        let plain = F.genericTrigger(with(empty) { $0.trigger = F.TriggerConfig(type: .ticket) })
    ...
    func testSubmitLabel() {
        XCTAssertEqual(F.submitLabel(empty), "Start session (opens a PR)")
        XCTAssertEqual(F.submitLabel(with(empty) { $0.withRepo = false }), "Start session")
        ...
        XCTAssertEqual(F.submitLabel(with(empty) { $0.when = .schedule }), "Save session")
    }
