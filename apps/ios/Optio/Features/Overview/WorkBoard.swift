...
struct SessionsBoardSections: View {
    ...
    private var recurring: [WorkRow] { Array(feed.rows(in: .recurring).prefix(5)) }
    private var agents: [WorkRow] { Array(feed.rows(in: .agents).prefix(5)) }
        ...
        Section {
            if placeholder {
                ...
                StatStrip(items: [
                    ...
                    switch item.key {
                    case "recurring": router.openWork(.recurring)
                    case "agents": router.openWork(.agents)
                    default: router.openWork(.active)
                    }
                }
                .listRowInsets(EdgeInsets())
                .listRowBackground(Color.clear)
            ...
            if let error = feed.error {
                ErrorRow(error: error, what: "work") { Task { await feed.refresh() } }
                    .listRowBackground(Color.clear)
            }
        }
    ...
    private func miniList(_ title: String, rows: [WorkRow], view: WorkView, empty: String) -> some View {
        Section {
            if rows.isEmpty {
            ...
            SectionHeader(title: title, detail: "\(feed.count(in: view))") { router.openWork(view) }.textCase(nil)
        }
    }
}
