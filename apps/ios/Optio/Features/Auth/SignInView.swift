...
struct SignInView: View {
    /// `.first` is the root auth gate (no servers yet); `.add` pairs another server from
    /// a sheet and asks for a name + colour so the two are told apart everywhere.
    enum Mode { case first, add }
    ...
    @State private var color: ServerColor = .slate
    @State private var busy = false
    @State private var error: SignInError?
    @State private var showHelp = false
    @FocusState private var focus: Field?

    private enum Field { case server, token, name }

    init(mode: Mode = .first) {
        self.mode = mode
        if mode == .first {
            _serverURL = State(initialValue: UserDefaults.standard.string(forKey: "optio.lastServerURL") ?? "")
        }
    }
    ...
    var body: some View {
        if mode == .add {
            ...
            .onAppear { color = ServerColor.next(avoiding: session.servers.map(\.color)) }
        } else {
            form
        }
    }
    ...
    private var form: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                if mode == .add {
                    field("Name", systemImage: "laptopcomputer") {
                        TextField(normalizedURL.map(ServerProfile.defaultName(for:)) ?? "MacBook Pro", text: $name)
                            .textInputAutocapitalization(.words)
                            .autocorrectionDisabled()
                            .focused($focus, equals: .name)
                            .submitLabel(.next)
                            .onSubmit { focus = .server }
                    }
                    .padding(.bottom, 14)
                    VStack(alignment: .leading, spacing: 8) {
                        Label("Colour", systemImage: "circle.hexagongrid")
                            .font(.subheadline.weight(.medium))
                            .foregroundStyle(.secondary)
                        ServerColorPicker(selection: $color)
                    }
                    .padding(.bottom, 14)
                }
                ...
                Button {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text(busy ? "Connecting to \(hostLabel)…" : (mode == .add ? "Add server" : "Connect"))
                            .fontWeight(.semibold)
                            .lineLimit(1)
                    }
        ...
        .onAppear { if serverURL.isEmpty { focus = mode == .add ? .name : .server } }
    }

    // MARK: - Pieces

    private var addIntro: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Pair another Optio instance")
                .font(.title2.weight(.semibold))
            Text("A second laptop, a cluster, a teammate's box. The name and colour label its items wherever servers are shown together.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private func submit() async {
        ...
        defer { busy = false }
        do {
            try await session.addServer(url: url, token: token.trimmingCharacters(in: .whitespacesAndNewlines),
                                        name: mode == .add ? name : nil, color: mode == .add ? color : nil)
