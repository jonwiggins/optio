...
// The six sections of `session-form.tsx`, one Form `Section` each. They read
...
struct WhenSection: View {
    ...
    private var footer: String? {
        ...
        case .github, .slack, .linear: return "Event triggers run sessions on your machine. Each firing opens one in the chosen directory with the event's fields available as {{param}}s."
        }
    }
...
struct WhoSection: View {
    ...
    private var footer: String {
        ...
        if state.isTerminal {
            ...
            lines.append("Runs with the server's credentials. Parameters start from the repo's defaults and apply to this session only.")
        } else {
    ...
    private func meta(_ then: F.Then) -> (icon: String, title: String, subtitle: String) {
        ...
        case .exits:
            return ("rectangle.portrait.and.arrow.right", "Exit when done",
                    state.draft.withRepo ? "One turn of work; opens the PR, then finishes" : "One turn of work, then the session finishes")
        case .waitsForMe:
            ...
            DisclosureGroup(isExpanded: $state.more) {
                ...
                if state.kind == .repoTask {
                    NavigationLink {
                        ...
                        LabeledContent("Wait for", value: state.draft.dependsOn.isEmpty ? "Nothing" : "\(state.draft.dependsOn.count) session\(state.draft.dependsOn.count == 1 ? "" : "s")")
                    }
                }
            } label: {
                Text("More options")
...
/// Pick the sessions a Task waits on: a checklist pushed from "Wait for".
struct DependenciesPicker: View {
    ...
    var body: some View {
        List {
            if tasks.isEmpty {
                ContentUnavailableView("Nothing to wait on", systemImage: "link", description: Text("No other sessions are queued or running."))
            } else {
                Section {
                    ...
                    Text("This session starts only after every checked session completes.")
                }
            }
        }
