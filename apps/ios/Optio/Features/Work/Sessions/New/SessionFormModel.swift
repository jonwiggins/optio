...
// Port of `apps/web/src/components/session-form/model.ts`. Pure data + pure
...
enum SessionForm {
    ...
    static func whereOptions(_ d: Draft) -> [Choice<Where>] {
        [
            Choice(value: .cluster, disabled: d.when.isEvent ? "GitHub, Slack, and Linear events run sessions on your machine." : nil),
            Choice(value: .local),
        ]
    }
    ...
    static func thenOptions(_ d: Draft) -> [Choice<Then>] {
        ...
        let messages: String? =
            ...
            : d.when.isEvent ? "Persistent agents wake on messages, schedules, webhooks, and tickets."
    ...
    static func deriveKind(_ d: Draft) -> Kind {
        ...
        if d.when.isEvent { return .localBlueprint }
    ...
    static func describe(_ d: Draft, _ ctx: Context = Context()) -> [SentencePart] {
        ...
        let noun = d.then == .waitsForMessages ? "agent" : "session"
    ...
    static func submitLabel(_ d: Draft) -> String {
        ...
        if d.when != .manual { return "Save session" }
        switch d.then {
        case .exits: return d.withRepo ? "Start session (opens a PR)" : "Start session"
