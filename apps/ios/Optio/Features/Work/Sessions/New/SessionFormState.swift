...
struct SessionFormRepo: Identifiable, Hashable, Sendable {
    ...
    init?(_ raw: [String: AnyCodable]) {
        ...
        fullName = raw["fullName"]?.stringValue ?? SessionForm.shortRepo(repoUrl)
...
extension APIClient {
    ...
    /// How many sessions exist, for the "Session N" placeholder (`total` when the
...
final class SessionFormState {
    ...
    var autoName: String { "Session \((sessionCount ?? 0) + 1)" }
    ...
    func submit() async -> F.Created? {
        ...
        do {
            return try await SessionFormSubmitter(api: api).create(draft, repoUrl: effectiveRepoUrl, autoName: autoName)
        } catch {
            self.error = "Couldn't create the session: \(ErrorText.humanize(error))"
            return nil
        }
    }
}
