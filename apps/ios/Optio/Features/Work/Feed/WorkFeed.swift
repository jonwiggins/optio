...
// model can replace `WorkFeed.collect` without touching the screens.
// Pure — no networking — so `OptioTests/WorkFeedTests.swift` mirrors the
// web's `sessions-feed.test.ts`.

enum WorkSource: String, Hashable, Sendable, CaseIterable {
...
enum WorkStatus: Int, Hashable, Sendable, Comparable {
    case needsYou = 0, running, queued, waiting, scheduled, paused, failed, done

    static func < (a: WorkStatus, b: WorkStatus) -> Bool { a.rawValue < b.rawValue }

    static let active: Set<WorkStatus> = [.needsYou, .running, .queued, .waiting]
...
/// Saved filters over the feed (`WorkView`).
enum WorkView: String, Hashable, Sendable, CaseIterable, Identifiable {
...
enum WorkDestination: Hashable, Sendable {
    case task(String)
...
struct WorkRow: Identifiable, Hashable, Sendable {
    let key: String
    ...
    var destination: WorkDestination {
        switch source {
...
enum WorkFeed {
    ...
    static func inView(_ row: WorkRow, _ view: WorkView) -> Bool {
        switch view {
        case .active: return WorkStatus.active.contains(row.status)
    ...
    static func matches(_ row: WorkRow, query: String) -> Bool {
    ...
    static func count(_ rows: [WorkRow]) -> WorkCounts {
        var c = WorkCounts()
        for r in rows {
    ...
    static func taskStatus(_ state: String?) -> (WorkStatus, String) {
        let state = state ?? ""
        switch state {
    ...
    static func terminalStatus(_ t: TerminalRow) -> (WorkStatus, String) {
    ...
    static func agentStatus(_ a: AgentRow) -> (WorkStatus, String) {
        switch a.state {
