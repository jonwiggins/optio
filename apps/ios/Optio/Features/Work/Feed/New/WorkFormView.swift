...
extension Notification.Name {
    /// Work was created from the form; feeds that show it should refresh.
        ...
        ScrollViewReader { proxy in
            ...
            .safeAreaInset(edge: .bottom, spacing: 0) {
                SubmitBar(state: state, onCreated: onCreated) { field in
                    withAnimation(.snappy) { proxy.scrollTo(WorkFormAnchor.forField(field), anchor: .top) }
                }
            }
...
private struct SubmitBar: View {
    ...
    private var firstGap: WorkForm.SentenceField? {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Spacing.m) {
            Text(sentence)
                ...
                .accessibilityLabel(WorkForm.sentenceText(state.sentence))
    ...
    private func gap(_ s: String, _ field: WorkForm.SentenceField) -> AttributedString {
