...
extension APIClient {
    private struct UnifiedEnvelope: Decodable { var tasks: [WorkFeed.UnifiedRow] }
    private struct TerminalsEnvelope: Decodable { var terminals: [WorkFeed.TerminalRow] }
    private struct BlueprintsEnvelope: Decodable { var blueprints: [WorkFeed.BlueprintRow] }
    private struct PodSessionsEnvelope: Decodable { var sessions: [WorkFeed.PodSessionRow] }
    private struct AgentsEnvelope: Decodable { var agents: [WorkFeed.AgentRow] }
    private struct HostsEnvelope: Decodable { var hosts: [WorkFeed.HostRow] }
    ...
    func sessionsFeedSources() async throws -> WorkFeed.Sources {
...
@MainActor
@Observable
final class WorkFeedModel {
    private(set) var rows: [WorkRow] = []
    ...
    var counts: WorkCounts { WorkFeed.count(rows) }

    func rows(in view: WorkView, query: String = "") -> [WorkRow] {
        rows.filter { WorkFeed.inView($0, view) && WorkFeed.matches($0, query: query) }
    }

    func count(in view: WorkView) -> Int { rows.filter { WorkFeed.inView($0, view) }.count }

    func refresh() async {
        do {
            let sources = try await api.sessionsFeedSources()
            rows = WorkFeed.collect(sources)
            error = nil
        } catch {
            self.error = error
        }
