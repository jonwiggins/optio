...
vi.mock("../services/local-event-service.js", async () => {
  const actual = await vi.importActual<typeof import("../services/local-event-service.js")>(
    "../services/local-event-service.js",
  );
  return { ...actual, fireLocalEventTriggers: (...args: unknown[]) => mockFire(...args) };
});
vi.mock("../db/client.js", () => ({ db: {} }));
vi.mock("../services/local-blueprint-service.js", () => ({
  getBlueprint: vi.fn(),
...
beforeEach(async () => {
  ...
  resetSlackEventDedupe();
  app = await buildRouteTestApp(localIngressRoutes, { user: null });
});
