/**
 * The New session form, end to end: one draft per storage kind, driven
...
const nameInput = (page: Page) => page.getByPlaceholder(/^Session /);
...
test.describe("New session form creates every kind", () => {
  test("repo-task: a pod Task that opens a PR carries the runtime, prompt, and title", async ({
    ...
    await expect(submit(page)).toHaveText(/Start session \(opens a PR\)/);
  ...
  test("standalone now: a Job with no repo starts a run", async ({ page }) => {
    ...
    await expect(submit(page)).toHaveText(/^Start session$/);
    await submit(page).click();
  ...
  test("local-blueprint: a Linear-assigned automation on a machine keeps the base branch and event filters", async ({
    page,
  }) => {
    await open(page);
    await preset(page, "Open a PR").click();
    await when(page, "Linear").click();
    // An event trigger runs on your machine — the pod card is out.
    await expect(where(page, "Optio pod")).toBeDisabled();
    await pickMachine(page, "/Users/e2e/repos/e2e-repo");
    await page.getByRole("button", { name: "New branch", exact: true }).click();
    // "Assigned" is about you, so the login is required before submit.
    await prompt(page).fill("We were assigned {{ticketUrl}}. Triage it.");
    await nameInput(page).fill(named("linear automation"));
    ...
    expect(bp.sessionMode).toBe("headless");
    expect(bp.baseBranch).toBe("main");
    expect(bp.commandTemplate).toBe("We were assigned {{ticketUrl}}. Triage it.");
