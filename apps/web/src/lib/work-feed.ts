 ...
 * the five attributes the New work form asks for (When / Where / Who /
 ...
 * client-side. A server-side `/api/work` read model can replace
 * `collectWork` without touching the pages.
 */

import type { Then } from "@/components/work-form/model";
...
export interface WorkRow {
   ...
   * reopens its five answers in the work form.
   */
  recurring: boolean;
...
const ACTIVE: WorkStatus[] = ["needs_you", "running", "queued", "waiting"];
...
function agentStatus(a: any): [WorkStatus, string] {
  switch (a.state) {
    case "running":
    case "provisioning":
