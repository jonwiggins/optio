...
import { countSessions, inView, type SessionRow, type SessionView } from "@/lib/sessions-feed";

/**
 * The overview's centre: one board over the unified sessions feed. Five
 * tiles (each a saved view of /sessions), then what's alive right now,
 * then the recurring and persistent sessions that will wake on their own.
 */
export function SessionsBoard({ rows, loading }: { rows: SessionRow[]; loading: boolean }) {
  const counts = countSessions(rows);
      ...
      <section className="rounded-xl border border-border/70 overflow-hidden">
        <header className="flex items-center justify-between px-4 py-2.5 bg-bg-card/60 border-b border-border/60">
          ...
          <div className="flex items-center gap-3 text-xs">
            <Link href="/sessions?view=active" className="text-text-muted hover:text-text">
              All →
            </Link>
            <Link
              href="/sessions/new"
