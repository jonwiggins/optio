...
**Tasks** — a configured unit of agent work. A Task has a **Who** (agent type), **What** (prompt or template), **When** (trigger: manual / schedule / webhook / ticket), **Where** (optional repo + branch, plus a run location: an Optio pod or a directory on the user's own machine via Optio Local), and **Why** (description). Tasks come in three flavors:
...
- **Persistent Agent** — long-lived, named, message-driven agent process that does _not_ terminate after running. Halts after each turn and waits to be re-woken by a user message, an agent message, a webhook, a cron tick, or a ticket event. Addressable by other agents in the same workspace via the inter-agent HTTP API (`/api/internal/persistent-agents/*`). Three configurable pod lifecycle modes: `always-on`, `sticky` (default, with idle warm window), and `on-demand`. UI at `/agents`. Schema: `persistent_agents`, `persistent_agent_turns`, `persistent_agent_messages`, `persistent_agent_pods`. See `docs/persistent-agents.md` and the demo in `examples/persistent-agents/forge/`.

**Scheduled (Task Configs)** — a saved Task blueprint that spawns fresh Tasks on a trigger firing. Stored in `task_configs`. Each firing calls `instantiateTask()` which goes through the full Repo Task pipeline. Managed from `/sessions?view=recurring` (detail at `/tasks/scheduled/:id`). Standalone equivalents are stored in `workflows` (see backend-naming note below).

**Triggers** — polymorphic table `workflow_triggers` keyed by `(target_type, target_id)`: `job` (Jobs), `task_config` (scheduled Tasks), `local_blueprint` (Local automations), `persistent_agent`, `pr_review`. The same seven types attach to any target: `manual`, `schedule` (cron), `webhook`, `ticket`, and the `github` / `slack` / `linear` events. One CRUD (`services/trigger-service.ts`) and one dispatcher (`services/trigger-dispatch.ts`, `fireTrigger`) serve every route and every firing path — the schedule poller, `/api/hooks/:path`, the ticket sweep, and the event ingress.
...
**v0.6 — one noun: Work.** The UI presents every one of these as **work** with five attributes — **When** (what starts it), **Where** (pod / your machine), **Who** (a terminal or an agent runtime + its parameters), **What** (the prompt), and **Then** (exit conditions: exits when done / waits for me / persistent agent) — plus a name. Work can be a one-time terminal on a pod or a machine, a one-off agent run, an agent handling a PR, or a recurring definition with prior runs and a history. The sidebar's top level is **Overview · Work · Reviews · Inbox**, then **Library** (Prompts · Repos · Machines · Connections); iOS has a Work tab with All / Reviews / Inbox under it. `/work` is the unified list (client-side merge of the per-kind endpoints in `lib/work-feed.ts`, views: Active / Recurring / Agents / History); `/work/new` is the one creation form (`components/work-form/`: `model.ts` derives which row to create — `deriveKind` — from the attributes, `submit.ts` calls the existing services and rolls a blueprint back if its trigger is rejected). **Every When works with every Where**: a GitHub / Slack / Linear event can start a Job or a scheduled Task in a pod as well as an automation on a machine; the form attaches the same trigger row whatever the kind. `/tasks/new`, `/jobs/new`, `/agents/new`, and the v0.5 `/sessions`, `/sessions/new`, `/sessions/:id/edit` redirect to their `/work` equivalents. `apps/web/e2e/work-form.spec.ts` drives every kind through the form against the real API. The per-kind **list** pages (`/tasks`, `/jobs`, `/agents`, `/local`, `/tasks/scheduled`) are gone — each redirects to the matching Work view — while the per-kind **detail** pages (`/tasks/:id`, `/jobs/:id`, `/tasks/scheduled/:id`, `/agents/:id`, `/local/:id`, `/sessions/:id` for pod sessions) remain and link back to `/work`. **Recurring work is edited in the same form.** A Recurring row opens the page _about_ the definition (stats, triggers, prior runs: `/tasks/scheduled/:id`, `/jobs/:id`, or `/local/automations/:id` for Local automations) and carries a pencil to `/work/:id/edit` (`components/work-form/load.ts` resolves the id across `task_configs` / `workflows` / `local_blueprints` and turns the row back into a draft; `submit.ts`'s `updateWork` PATCHes the row and syncs its trigger). The saved kind is locked while editing (`kindLock` in `model.ts`): answers that would move the row to another table are disabled with a reason. `/jobs/:id/edit` redirects there; the Machines automations list and the `/tasks/scheduled/:id` page link there for edits. Legacy `/tasks?tab=...` URLs redirect to `/work?view=recurring`, `/issues`, `/reviews`. "Session" keeps only its narrow meaning: an interactive pod session (`/sessions/:id`, `interactive_sessions`).
...
- **Standalone Tasks / Jobs** (`workflow-service.ts`, `workflow-worker.ts`): listed in `/sessions?view=recurring` (detail at `/jobs/:id`, runs at `/jobs/:id/runs/:runId`). Agent runs with no repo, `{{PARAM}}` prompt templates, four trigger types (manual/schedule/webhook/ticket), pooled pod execution, real-time log streaming, auto-retry with exponential backoff. Pods are **shared across runs within a workflow**, keyed on `(workflow_id, instance_index)`: each workflow has `workflows.maxPodInstances` pod replicas (default 1, max 20) and `workflows.maxAgentsPerPod` concurrent runs per pod (default 2, max 50) — mirrors repo pod scaling. Runs track their assigned pod via `workflow_runs.pod_id` and remember it for retry affinity via `last_pod_id`. Schema: `workflows`, `workflow_triggers`, `workflow_runs`, `workflow_run_logs`, `workflow_pods`
- **Repo Task Configs** (`task-config-service.ts`, routes in `task-configs.ts`): reusable Repo Task blueprints that spawn tasks when triggers fire. `instantiateTask(configId, { triggerId, params })` creates a task with rendered prompt + title, transitions it to QUEUED, and enqueues the BullMQ job. UI at `/sessions?view=recurring` + `/tasks/scheduled/:id`. Schema: `task_configs`
- **Triggers** (`trigger-service.ts`, `trigger-dispatch.ts`, `event-trigger-service.ts`, `workflow-trigger-worker.ts`, shared bodies in `schemas/trigger.ts`): polymorphic trigger table (`workflow_triggers`) keyed by `(target_type, target_id)`. `trigger-service` is the one CRUD every route uses (`listTriggers` / `createTrigger` / `updateTrigger` / `deleteTrigger`, `validateTriggerConfig`, webhook-path uniqueness, schedule `next_fire_at`); `TRIGGER_TYPES_FOR_TARGET` in `@optio/shared` says which types a target takes (all seven for `job` / `task_config` / `local_blueprint` / `persistent_agent`, `schedule` only for `pr_review`). `trigger-dispatch.fireTrigger(trigger, firing)` starts what the target starts (`createWorkflowRun` / `instantiateTask` / `spawnFromBlueprint` / `wakeAgent` / `reReview`) and stamps `last_fired_at`; the schedule worker (polls every 60s, `OPTIO_WORKFLOW_TRIGGER_INTERVAL`), the webhook ingress, `fireTicketTriggers` (ticket sweep), and `fireEventTriggers` (GitHub / Slack / Linear ingress; matching in `event-trigger-service.ts`) all go through it. A scheduled Task with a `github` trigger and no `repos` filter listens to its own repo; a task started by a PR / issue event is not ticket-linked (that would auto-close the issue).
...
- **Optio Local** (`local-host-service.ts`, `local-terminal-service.ts`, `local-blueprint-service.ts`, `local-relay.ts`, ws in `ws/local-daemon.ts` + `ws/local-terminal-stream.ts`, routes in `routes/local.ts`, daemon in `apps/cli/src/local/`): terminals on the **user's own machine**, managed from the web UI at `/local`. The CLI daemon (`optio local up`) makes one outbound WS to `/ws/local/daemon` (PAT auth), advertises a dir allowlist, and runs node-pty PTYs; the in-process relay (`local-relay.ts`, single API replica) routes frames to browser viewers on `/ws/local/terminals/:id/stream` (binary = terminal bytes, JSON = control). Attention detection is daemon-side and layered: Claude Code hooks (Stop/Notification/UserPromptSubmit via a localhost hook server) → OSC-aware bell scanner → silence heuristic; persisted as `attention_state` (`working`/`needs_you`/`idle`) driving the cockpit's "needs you" queue. **Local Automations** (`local_blueprints`, "blueprints" in code) are agent / terminal specs spawned by `workflow_triggers` with `target_type="local_blueprint"`: any of the seven trigger types incl. the events `github` / `slack` / `linear` (signed ingress in `routes/event-ingress.ts` + the existing GitHub receiver; matching in `event-trigger-service.ts`, the same path that fires pod-side Jobs and scheduled Tasks). `sessionMode` picks `interactive` (agent halts at its prompt for chat) or `headless` (`claude -p` etc., exits when done); the daemon reports the agent's `session_id` so an exited run can be resumed (`POST /api/local/terminals/:id/resume`). Trigger params are shell-single-quoted before `{{param}}` substitution — payloads can never inject commands. Hosts are per-user (never workspace compute); local agents use the machine's own CLI auth (no server secrets ship to laptops). **Local runs** (`local-run-service.ts`): every Task, Job, and scheduled-Task blueprint has a run location (`run_target` = `cluster` | `local`, plus `local_host_id` / `local_dir` / `local_session_mode`) picked with the shared `components/run-location-picker.tsx`; the task / workflow workers dispatch `local` runs to the daemon as an agent terminal (`spawned_by = "job" | "task"`, `local_terminal_id` on the run) instead of a pod, and the terminal's frames drive the run's state (`syncLinkedRun`: running, usage → cost, PR link → `pr_opened`, exit → completed / failed). The reconciler skips capacity / stall / pod checks for local runs. Schema: `local_hosts`, `local_terminals`, `local_blueprints`. See `docs/optio-local.md`
