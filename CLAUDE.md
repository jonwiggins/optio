...
**v0.5 — one noun: Sessions.** The UI now presents every one of these as a **session** with five attributes — **When** (what starts it), **Where** (pod / your machine), **Who** (a terminal or an agent runtime + its parameters), **What** (the prompt), and **Then** (exit conditions: exits when done / waits for me / persistent agent) — plus a name. The sidebar is **Work** (Sessions · Reviews · Inbox) and **Library** (Prompts · Repos · Machines · Connections). `/sessions` is the unified list (client-side merge of the per-kind endpoints in `lib/sessions-feed.ts`, views: Active / Recurring / Agents / History); `/sessions/new` is the one creation form (`components/session-form/`: `model.ts` derives which row to create — `deriveKind` — from the attributes, `submit.ts` calls the existing services and rolls a blueprint back if its trigger is rejected). `/tasks/new`, `/jobs/new`, and `/agents/new` redirect there. `apps/web/e2e/session-form.spec.ts` drives every kind through the form against the real API. The per-kind **list** pages (`/tasks`, `/jobs`, `/agents`, `/local`, `/tasks/scheduled`) are gone — each redirects to the matching Sessions view — while the per-kind **detail** pages (`/tasks/:id`, `/jobs/:id`, `/tasks/scheduled/:id`, `/agents/:id`, `/local/:id`, `/sessions/:id`) remain and link back to `/sessions`. Local Automations are edited on `/machines#automations`. Legacy `/tasks?tab=...` URLs redirect to `/sessions?view=recurring`, `/issues`, `/reviews`.
